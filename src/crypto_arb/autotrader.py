...
class AutoTrader:
    ...
    async def execute_trade(self, trade_info: dict) -> dict:
        ...
        try:
            ...
            # Usar el mismo order_price (con slippage) que el FOK — ese precio agresivo
            # cruza el spread y permite que la GTC se llene como taker.
            if fok_failed:
                try:
                    print(f"[AutoTrader] GTC fallback: price={order_price} shares={shares} "
                          f"usdc={round(shares*order_price,2)}", flush=True)
